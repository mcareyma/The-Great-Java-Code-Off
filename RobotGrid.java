...
public class RobotGrid {
    ...
    public static int totalSafeSquares() {
        int count = 0;
        boolean[][] visited = new boolean[1400][1400]; // Keep track of visited squares
        ...
        while (!queue.isEmpty()) {
            ...
            if (isSafe(x, y)) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    if (nx >= -700 && nx <= 700 && ny >= -700 && ny <= 700 && !visited[nx+700][ny+700]) {
                        queue.offer(new int[]{nx, ny});
                        visited[nx+700][ny+700] = true;
                    }
                }
            }
        }
